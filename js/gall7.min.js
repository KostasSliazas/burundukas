(function(p,l){p.onload=function(){function d(b){return l.createElement(b)}function q(b){const c=arguments;for(let f=1;f<c.length;f+=2)b.setAttribute(c[f],c[f+1])}function k(b){const c=arguments;for(let f=1;f<c.length;f++)b.appendChild(c[f])}function r(b){if(a.isActive&&!b.isComposing&&229!==b.key){var c=b.key||b.target.id;if(!h[c])return a.clear();h[c]();b.preventDefault();b.stopImmediatePropagation()}}var e="undefined"!==typeof p.IGConfig&&p.IGConfig,g=d("link");q(g,"rel","stylesheet","href","data:text/css;base64,");
k(l.getElementsByTagName("head")[0],g);const a={};a.S=e.folder||"x/";a.R=e.extension||"";a.J=e.imageContainer||"gall7";a.V=isFinite(e.delaySeconds)?1E3*e.delaySeconds:1700;a.N="undefined"===typeof e.showButtonsOnPlay?!0:!!e.showButtonsOnPlay;a.v="undefined"===typeof e.showButtons?!0:!!e.showButtons;a.j=[];a.B=!1;a.isActive=!1;a.h=0;a.O=0;a.m=d("button");a.I=d("button");a.K=d("button");a.i=d("div");a.C=d("div");a.left=d("div");a.u=d("div");a.l=d("div");a.C.id="cent7";a.I.id="ilef7";a.K.id="irig7";
a.m.id="clos7";a.u.id="rigt7";a.l.id="insi7";a.left.id="left7";a.i.id="imag7";a.i.className="hide7";k(a.C,a.l,a.u,a.left,a.m);k(a.u,a.K);k(a.left,a.I);k(a.i,a.C);q(a.m,"aria-label","Close","title","Press Esc to close");k(l.body,a.i);a.G=function(){this.B=!0;this.v&&(this.play.className="acts7");this.O=setTimeout(function(){this.right().show();this.N||(this.left.className=this.u.className=this.m.className="hide7",this.v&&(this.o.className=this.s.className="hide7"));this.h===this.j.length-1&&this.clear()}.bind(this),
this.V)};a.U=function(){this.l.className="";this.B&&this.G()};a.P=function(){const b=d("a");q(b,"rel","noopener","download",this.g.src.split("/").pop(),"href",this.g.src,"target","_blank");b.click();b.remove()};a.T=function(){0<this.h?this.h--:this.h=this.j.length-1;return this};a.right=function(){this.h<this.j.length-1?this.h++:this.h=0;return this};a.clear=function(){clearTimeout(this.O);this.B=!1;this.v&&(this.o.className=this.s.className=this.play.className="");this.N||(this.m.className="");this.L();
return this};a.close=function(){this.i.className="hide7";this.isActive=!1;l.documentElement.style.overflow="visible"};a.L=function(){this.left.className=0===this.h?"hide7":"";this.u.className=this.h===this.j.length-1?"hide7":""};a.show=function(){const b=this.j[this.h],c=b.src,f=c.split("/").pop();var m=f.split(".");m=0<f.indexOf(".svg")?c:c.replace(f,this.S+m[0]+"."+(this.R||m[1]));this.isActive||(this.isActive=!0,document.documentElement.style.overflow="hidden",this.i.className="");this.v&&(this.A.innerText=
decodeURI(m.split("/").pop()),this.D.innerText=Number(this.h)+1);if(this.g&&(this.g.src===c||this.g.src===m))return!1;this.g&&this.l.removeChild(this.g);this.l.className="spin7";this.L();0<f.indexOf(".svg")?(this.g=b.cloneNode(),this.g.removeAttribute("id"),this.g.removeAttribute("class")):this.g=d("img");this.g.onerror=function(t){t.target.onerror=null;t.target.src=c};this.g.onload=function(){this.U();this.A.innerText=decodeURI(this.g.src.split("/").pop())}.bind(this);k(this.l,this.g);this.g.src=
m};a.M=function(b){const c=b.target;"IMG"===c.tagName&&(this.h=-1<this.j.indexOf(c)?this.j.indexOf(c):0,this.show(),b.stopImmediatePropagation())};g=0<l.getElementsByClassName(a.J).length?l.getElementsByClassName(a.J):l.getElementsByTagName("body");e=[];for(var n=g.length-1;0<=n;n--)e.push(g[n]);for(g=e.length-1;0<=g;g--){n=e[g].getElementsByTagName("img");for(let b=0;b<n.length;b++)a.j.push(n[b])}if(e[0]&&"BODY"===e[0].tagName)l.body.onclick=function(b){a.M(b)};else for(g=e.length-1;0<=g;g--)e[g].onclick=
function(b){a.M(b)};a.v&&(a.F=d("button"),a.play=d("button"),a.o=d("div"),a.s=d("div"),a.A=d("span"),a.D=d("span"),a.H=d("span"),a.A.id="alts7",a.play.id="play7",a.o.id="foot7",a.s.id="onow7",a.H.id="down7",a.F.id="wdow7",a.D.id="stat7",k(a.s,a.A,a.F),k(a.i,a.s,a.o),k(a.o,a.play,l.createTextNode(a.j.length+"("),a.D,l.createTextNode(")")),k(a.F,a.H));const h={play7:function(){a.B?a.clear():a.G()},left7:function(){a.clear().T().show()},rigt7:function(){a.clear().right().show()},clos7:function(){a.clear().close()},
wdow7:function(){a.clear().P()}};h[" "]=h.play7;h.ArrowLeft=h.left7;h.ArrowRight=h.rigt7;h.Escape=h.clos7;let u,v,w;a.i.addEventListener("touchstart",function(b){u=b.touches[0].pageX;v=b.touches[0].pageY;w=new Date},{passive:!0});a.i.addEventListener("touchend",function(b){const c=b.changedTouches[0].pageX-u;b=b.changedTouches[0].pageY-v;const f=new Date-w;30<Math.abs(c)&&30>Math.abs(b)&&1E3>f&&(0>c?h.rigt7():h.left7())});p.addEventListener("keyup",r);a.i.onclick=r}})(window,document);
